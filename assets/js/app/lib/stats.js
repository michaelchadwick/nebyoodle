...
Nebyoodle.__getSessionCount = function (mode) {
  ...
  if (state) {
    return Nebyoodle.__getStateObj(mode).filter((session) => Nebyoodle.___isDone(session)).length
  } else {
    return 0
  }
}
...
Nebyoodle.__getSuccessPerc = function (mode) {
  ...
  const winCount = Nebyoodle.__getStateObj(mode).filter((session) =>
    Nebyoodle.___isWin(session)
  ).length
...
Nebyoodle.__getStreak = function (mode, max = null) {
  ...
  Nebyoodle.__getStateObj(mode).forEach((session) => {
